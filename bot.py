...
SMTP_PORT      = int(os.environ.get("SMTP_PORT", "465"))   # 465 = SSL direct
TIMEOUT_SEC    = 60
...
def _smtp_send(to: str, subject: str, body: str) -> None:
    logger.info(f"SMTP → {SMTP_HOST}:{SMTP_PORT} | to={to}")
    msg = EmailMessage()
    ...
    logger.info(f"SMTP envoi OK → {to}")
...
async def handle_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "e:send":
        ...
        try:
            ...
            logger.error(f"SMTP auth failed uid={uid}")
            await edit_main(
                context.bot, chat_id, msg_id,
                "❌ Échec : authentification SMTP refusée.\n\n"
                "Vérifie que l'accès SMTP est activé dans ton compte GMX\n"
                "(Paramètres → POP3/IMAP → Activer).",
                RETRY_KB,
            )
        except TimeoutError:
            logger.error(f"SMTP timeout uid={uid}")
            await edit_main(
                context.bot, chat_id, msg_id,
                "❌ Échec : connexion SMTP expirée.\n\n"
                f"Hôte : {SMTP_HOST}:{SMTP_PORT}",
                RETRY_KB,
            )
        except Exception as exc:
            logger.error(f"SMTP erreur uid={uid} : {exc}")
            await edit_main(
                context.bot, chat_id, msg_id,
